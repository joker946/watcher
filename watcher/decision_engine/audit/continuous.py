...
WATCHER_CONTINUOUS_OPTS = [
    cfg.IntOpt('period_interval',
               default=10,
               help='The frequency (in seconds) continuous'
                    'audits is called with.')
]
...
class ContinuousAuditHandler(base.AuditHandler):
    ...
    def launch_audits_periodically(self):
        ...
        audit_filters = {
            ...
            'state_in': (audit_objects.State.PENDING,
        ...
        for audit in audits:
            if audit.uuid not in [arg[0].uuid for arg in scheduler_job_args]:
                job = self.scheduler.add_job(self.execute_audit, 'interval',
                                             args=[audit, audit_context],
                                             seconds=audit.period,
                                             name='execute_audit')
    ...
    def _start(self):
        self.scheduler.add_job(
            ...
            'interval',
            seconds=CONF.watcher_decision_engine.period_interval)
